...
nohup conda run -n $ENV_NAME && pip install gunicorn && gunicorn -w 1 -k uvicorn.workers.UvicornWorker -b 0.0.0.0:$port -t 1800 --log-level 'debug' server:app > $LOG_NAME 2>&1 &
#14分钟启动
