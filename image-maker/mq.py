...
def image_to_base64(file_path):
    ...
    try:
        ...
        logger.info(f"Successfully converted {file_path} to Base64.")
...
def send_message(message_body,channel):
    ...
    try:
        ...
        logger.info("Message sent successfully.")
            ...
            logger.warning(f"Failed to publish message: {e}. Retrying in 5 seconds...")
...
def get_image_remote(prompt: str, step: int = 4):
    ...
    if step < 4:
        step = 50
    img = generate(prompt, step, 3.5, 1024 , 768, -1)
...
def callback(ch, method, properties, body):
    ...
    try:
        ...
        logger.info(f"File {img_path} has been deleted successfully.")
    except FileNotFoundError:
        logger.error(f"File {img_path} does not exist.")
    except PermissionError:
        logger.error(f"No permission to delete file {img_path}.")
    except Exception as e:
...
def consume_messages():
    ...
    try:
        ...
        logger.warning(f"Connection lost (StreamLostError): {e}. Reconnecting in 5 seconds...")
